...
public class GameTree {
    ...
    int findValue() {
        ...
        if (root == null) {
            ...
            fillValue(root, Player.ONE);
            return root.value;
        }

    }

    // fills in value of nodes by traversing down to leaves then going back up (max or min depending on player)
    void fillValue(TreeNode node, Player player) {
        if (node.leaf) {
            // do nothing as leaves alr have values
        } else {
            // recurse down to leaves
            for (TreeNode n : node.children) {
                if (player == Player.ONE) {
                    fillValue(n, Player.TWO);
                } else {
                    fillValue(n, Player.ONE);
                }
            }

            // player one nodes take maximum
            if (player == Player.ONE) {
                int max = Integer.MIN_VALUE;
                for (int i = 0; i < node.numChildren; i++) {
                    if (max < node.children[i].value) {
                        max = node.children[i].value;
                    }
                }
                node.value = max;
            // player two nodes take minimum
            } else {
                int min = Integer.MAX_VALUE;
                for (int i = 0; i < node.numChildren; i++) {
                    if (min > node.children[i].value) {
